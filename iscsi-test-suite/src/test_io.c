...
static test_result_t test_maximum_transfer_size(struct iscsi_context *unused_iscsi,
    ...
    uint32_t block_size;
    uint8_t *write_buf, *read_buf;
    /* Test at MaxBurstLength boundary (default: 262144 bytes = 512 blocks of 512 bytes) */
    const size_t max_burst_length = 262144;
    uint32_t num_test_blocks;
    ...
    num_test_blocks = max_burst_length / block_size;
    if (num_test_blocks == 0) {
        report_set_result(report, TEST_SKIP, "Block size too large for MaxBurstLength test");
        iscsi_disconnect_target(iscsi);
        iscsi_destroy_context(iscsi);
        return TEST_SKIP;
    }
    ...
    /* Write data at MaxBurstLength boundary */
...
static test_result_t test_beyond_maximum_transfer(struct iscsi_context *unused_iscsi,
    ...
    uint32_t block_size;
    uint8_t *write_buf, *read_buf;
    /* Test beyond MaxBurstLength (default: 262144 bytes) - use 2x to exceed */
    const size_t max_burst_length = 262144;
    const size_t test_size = max_burst_length * 2; /* 524288 bytes */
    uint32_t num_test_blocks;
    ...
    /* Calculate number of blocks for 2x MaxBurstLength */
    num_test_blocks = test_size / block_size;
    if (num_test_blocks == 0) {
        report_set_result(report, TEST_SKIP, "Block size too large for beyond MaxBurstLength test");
        iscsi_disconnect_target(iscsi);
        iscsi_destroy_context(iscsi);
        return TEST_SKIP;
    }
    ...
    if (num_blocks < num_test_blocks + 15000) {
        report_set_result(report, TEST_SKIP, "Insufficient capacity for beyond MaxBurstLength test");
        iscsi_disconnect_target(iscsi);
        iscsi_destroy_context(iscsi);
        return TEST_SKIP;
    }
    ...
    /* Write data exceeding MaxBurstLength - target should handle via R2T */
    generate_pattern(write_buf, total_size, "sequential", 11011);
    ...
    /* Read and verify - target should have handled the large write correctly */
    if (scsi_read_blocks(iscsi, config->lun, 15000, num_test_blocks, block_size, read_buf) != 0) {
        report_set_result(report, TEST_FAIL, "Read after beyond MaxBurstLength write failed");
        free(write_buf);
        free(read_buf);
        iscsi_disconnect_target(iscsi);
        iscsi_destroy_context(iscsi);
